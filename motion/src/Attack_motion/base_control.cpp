...
//    for(int i = 0; i < length; i++){
//	    count_buffer = (count_buffer)%buffer_size;
...
////            printf("%x\n", crc_16);
//			if(crc_16 == 0){
//				base_RX.head1 = length;
//				base_RX.head2 = cssl_buffer[(i+2)%buffer_size] << 8 + cssl_buffer[(i+3)%buffer_size];
//				base_RX.w1 = (cssl_buffer[(i+4)%buffer_size]<<8)+(cssl_buffer[(i+5)%buffer_size]);
//				base_RX.w2 = (cssl_buffer[(i+6)%buffer_size]<<8)+(cssl_buffer[(i+7)%buffer_size]);
//				base_RX.w3 = (cssl_buffer[(i+8)%buffer_size]<<8)+(cssl_buffer[(i+9)%buffer_size]);
//				base_RX.shoot = 0;
//				base_RX.batery = 0;
//                base_flag = true;
//				break;
