...
bool NubotGazebo::update_model_info(void)
{
    if(ModelStatesCB_flag_)
    {
        ...
        for(int i=0; i<model_count_;i++)
        {
            ...
            {
                ignition::math::Vector3d obs_pos(model_states_.pose[i].position.x,
                ...
                if(i != robot_index_)
                {
                    ...
                    ignition::math::Vector3d nubot_obs_vec = obs_pos - robot_state_.pose.position;   // vector from nubot to obstacle
